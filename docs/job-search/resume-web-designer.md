# Résumé — Web Designer (AI-assisted) · Remote

**Kunal Varshney**  
India · Remote · kunalvrshn@gmail.com · +91 85340 49747  
Portfolio: https://lilkunal.github.io/ · LinkedIn: linkedin.com/in/kunalvrshn/ · GitHub: https://github.com/lilkunal

---

## Summary

Web designer who builds and launches business websites using an **AI-assisted workflow** (Claude, Cursor) — faster delivery without skipping QA, accessibility, or post-launch support. Live client work plus nine years operating Padma Lights online (200+ products). Four years of technical support before design, so sites stay maintained after go-live.

---

## Core skills

HTML · CSS · JavaScript · Responsive UI · Landing pages · SEO basics · Photoshop · Git / GitHub Pages · AI-assisted development (Claude, Cursor) · Client communication · Post-launch support

---

## Experience

**Freelance Web Designer** · Remote · Jan 2025 – Present  
- Design and build marketing sites and landing pages for small businesses.  
- **AI-assisted workflow:** use Claude and Cursor for layout, components, and iteration; personally review code, copy, mobile behaviour, and launch checklist.  
- Clients: JAI Home Care (home-care landing page), this portfolio, and other small businesses.  
- Scope, timeline, and pricing agreed in writing; support after launch.

**Marketing & Online Specialist** · Padma Enterprises (Padma Lights) · Jan 2025 – Present  
- Lead website, catalogue, and digital marketing for cast-aluminium outdoor lighting.  
- Manage 200+ model catalogue, trade pricing, international orders (8 currencies).  
- Ongoing site updates, product imagery, and export-facing online operations.

**Technical Support Advisor** · Teleperformance · Aug 2024 – Dec 2024  
**Customer Support Executive** · Webhelp · Oct 2023 – Jan 2024  
**Content Moderator** · Concentrix · Oct 2021 – Apr 2023  
**Technical Support Trainee** · Flex · Feb 2021 – Jun 2021  

---

## Selected projects

| Project | URL | Role |
|---------|-----|------|
| Padma Lights | padmalights.com | Marketing, catalogue, export ops |
| JAI Home Care | jaihomecareservice-max.github.io/Jai-home-care/ | Design & build |
| Portfolio | lilkunal.github.io | Design, build, AI-assisted |

---

## Education & certs

B.Tech in Computer Science · Aligarh College of Engineering & Technology · 2020  
Google IT Support Professional Certificate · Google Digital Garage (Digital Marketing) · ChatGPT for Beginners (Great Learning)

---

## One-line pitch for applications

*Web designer, remote-ready — I ship mobile-first business sites with an AI-assisted build process (Claude/Cursor), real client proof on padmalights.com, and four years of support so nothing breaks after launch.*
